# ADR-000: Architecture Decision Record Index

## Accepted ADRs

| ADR | Title | Status |
|---|---|---|
| ADR-001 | Relationship-First Architecture | Accepted |
| ADR-002 | Operational Source of Truth | Accepted |
| ADR-003 | Trust but Verify Change Processing | Accepted |
| ADR-004 | Identity and Account Separation | Accepted |
| ADR-005 | Event-Driven Operational Model | Accepted |
| ADR-006 | Human-Centered Security Operations | Accepted |
| ADR-007 | Operational Journal | Accepted |
| ADR-008 | Continuous Evidence | Accepted |
| ADR-009 | Engine-First Architecture | Accepted |
| ADR-010 | Base Model Standardization | Accepted |
| ADR-011 | Containerization Strategy | Accepted |
| ADR-012 | Database Strategy | Accepted |
| ADR-013 | Identity Domain Model | Accepted |
| ADR-014 | Shared Domain Base Classes | Accepted |
| ADR-015 | Access Governance Suite | Accepted |